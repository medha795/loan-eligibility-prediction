...
Below is the full flow of how raw LendingClub data becomes the 
`engineered_loan_dataset.csv` used for modeling.

                         ┌────────────────────────────────────┐
                         │  Raw LendingClub Accepted /        │
                         ...
                         └────────────────────────────────────┘
                                       ...
                                       ▼
                 ┌─────────────────────────────────────────────────────┐
                 │  Feature Engineering Pipeline (prepare_dataset.py)  │
                 └─────────────────────────────────────────────────────┘
                                       │
                                       ▼
              ┌─────────────────────────────── Chunked Processing ────────────────────────────────┐
              │                                                                                   │
              │   1. Load large initial sample (e.g., 500k rows)                                  │
              │   2. Normalize column names                                                       │
              │   3. Parse text → numeric:                                                        │
              │        • term → term_months                                                       │
              │        • emp_length → emp_length_years                                            │
              │        • int_rate%, dti% → float                                                  │
              │        • fico_score from (low+high)/2                                             │
              │   4. Clean categorical fields (trim/lowercase)                                    │
              │   5. Impute missing values                                                        │
              │   6. One-hot encode categorical features                                          │
              │   7. Map loan_status → {accepted:1, rejected:0}                                   │
              │   8. Derive the **full schema** from this sample                                  │
              │                                                                                   │
              └─────────────────────────────────────────────────────────────────────────────────────┘
                                       │
                                       ▼
             ┌──────────────────────────── Chunked File Iteration  ──────────────────────────────┐
             │                                                                                 │
             │   • Read dataset in chunks (e.g., 100k rows)                                   │
             │   • Apply engineer_features() to each chunk                                     │
             │   • Reindex chunk to match established schema                                   │
